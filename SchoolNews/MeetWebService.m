...
@implementation MeetWebService
@synthesize delegate;
MeetWebService* g_MeetWebService;
+ (id )singleton
{
    if (g_MeetWebService == nil) {
        g_MeetWebService = [[MeetWebService alloc] init];
    }
    return g_MeetWebService;
}
...
- (void)doFinishGetTheWebServiceDataWithObject:(id)object
{
    /*[[NSNotificationCenter defaultCenter] postNotificationName:MEET_END_UPDATE_WITH_WEBSERVICE
                                                        object:object];*/
    [self.delegate finishGetMeetData:object];
}
@end
