...
@implementation EmploymentWebservice
@synthesize delegate;
EmploymentWebservice* g_EmploymentWebservice;
+ (EmploymentWebservice* )singleton
{
    if (g_EmploymentWebservice == nil) {
        g_EmploymentWebservice = [[EmploymentWebservice alloc] init];
    }
    return g_EmploymentWebservice;
}
//模板方法
//处理webservice收到的数据
...
{
    [self.delegate finishGetEmploymentData:object];
}
